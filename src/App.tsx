...
import {
  generateReligionQuestion,
  type ReligionQuestion,
} from './utils/religionQuestions';
import {
...
const defaultHistoryScopes: HistoryScope[] = ['grade3-h2'];
const defaultReligionScopes: ReligionScope[] = ['grade1-h1'];
const defaultScheduler = createQuizSchedulerState();
const AUTO_ADVANCE_MS = 3000;
type Subject = 'countries' | 'history' | 'religion';
type ActiveQuestion = Question | HistoryQuestion | ReligionQuestion;
type TextQuestion = HistoryQuestion | ReligionQuestion;

const isTextQuestion = (question: ActiveQuestion): question is TextQuestion =>
  'kind' in question &&
  (question.kind === 'history' || question.kind === 'religion');
...
const App = () => {
  ...
  const [selectedHistoryScopes, setSelectedHistoryScopes] =
    useState<HistoryScope[]>(defaultHistoryScopes);
  const [selectedReligionScopes, setSelectedReligionScopes] = useState<
    ReligionScope[]
  >(defaultReligionScopes);
  ...
  const filteredHistoryFacts = useMemo(
    () =>
      historyFacts.filter((fact) => selectedHistoryScopes.includes(fact.scope)),
    [selectedHistoryScopes],
  );
  const filteredReligionFacts = useMemo(
    () =>
      religionFacts.filter((fact) =>
        selectedReligionScopes.includes(fact.scope),
      ),
    [selectedReligionScopes],
  );
  ...
  const allHistoryScopesSelected =
    selectedHistoryScopes.length === historyScopeOptions.length;
  const selectedHistoryScopeSummary = allHistoryScopesSelected
    ? 'Všetko'
    : historyScopeOptions
        .filter((scope) => selectedHistoryScopes.includes(scope.id))
        .map((scope) => scope.label)
        .join(', ');
  const allReligionScopesSelected =
    selectedReligionScopes.length === religionScopeOptions.length;
  const selectedReligionScopeSummary = allReligionScopesSelected
    ? 'Všetko'
    : religionScopeOptions
        .filter((scope) => selectedReligionScopes.includes(scope.id))
        .map((scope) => scope.label)
        .join(', ');

  const generateSubjectQuestion = useCallback(
    (
      scheduler: ReturnType<typeof createQuizSchedulerState>,
    ): ActiveQuestion => {
      if (subject === 'countries') {
        return generateScheduledQuestion(
          scheduler,
          filteredCountries,
          questionMode,
        );
      ...
      if (subject === 'history') {
        return generateHistoryQuestion(filteredHistoryFacts);
      }

      return generateReligionQuestion(filteredReligionFacts);
    },
    [
      filteredCountries,
      filteredHistoryFacts,
      filteredReligionFacts,
      questionMode,
      subject,
    ],
  );
  ...
  useEffect(() => {
    ...
    setResult('');
    setQuestion(generateSubjectQuestion(nextScheduler));
  ...
  const next = useCallback(() => {
    setQuestion(generateSubjectQuestion(quizScheduler));
    setResult('');
    setSelectedAnswer(null);
    setAutoAdvanceProgress(0);
  }, [generateSubjectQuestion, quizScheduler]);
  ...
  const toggleAllHistoryScopes = () => {
    setSelectedHistoryScopes(
      allHistoryScopesSelected
        ? defaultHistoryScopes
        : historyScopeOptions.map((scope) => scope.id),
    );
    hardResetProgress();
  };

  const toggleHistoryScope = (scope: HistoryScope) => {
    if (
      selectedHistoryScopes.includes(scope) &&
      selectedHistoryScopes.length === 1
    ) {
      return;
    }
    ...
    setSelectedHistoryScopes(
      selectedHistoryScopes.includes(scope)
        ? selectedHistoryScopes.filter(
            (selectedScope) => selectedScope !== scope,
          )
        : [...selectedHistoryScopes, scope],
    );
    hardResetProgress();
  };

  const toggleAllReligionScopes = () => {
    setSelectedReligionScopes(
      allReligionScopesSelected
        ? defaultReligionScopes
        : religionScopeOptions.map((scope) => scope.id),
    );
    hardResetProgress();
  };

  const toggleReligionScope = (scope: ReligionScope) => {
    if (
      selectedReligionScopes.includes(scope) &&
      selectedReligionScopes.length === 1
    ) {
      return;
    }
    ...
    setSelectedReligionScopes(
      selectedReligionScopes.includes(scope)
        ? selectedReligionScopes.filter(
            (selectedScope) => selectedScope !== scope,
          )
        : [...selectedReligionScopes, scope],
    );
    hardResetProgress();
  };
  ...
  const religionActive = subject === 'religion';
  ...
  return (
    <main className="mx-auto max-w-4xl p-4 text-slate-900">
      ...
      <div className="mb-4 flex flex-wrap items-stretch gap-2 rounded-xl bg-white p-2 shadow">
        ...
        <button
          className={`min-h-11 flex-1 self-stretch whitespace-nowrap rounded-lg border-2 px-3 py-2 text-base font-bold sm:flex-none sm:text-lg ${religionActive ? 'border-violet-700 bg-violet-500 text-white shadow' : 'border-transparent bg-violet-100 text-violet-900'}`}
          onClick={() => setSubject('religion')}
          aria-pressed={religionActive}
        >
          Náboženstvo
        </button>
        <button
          ...
          >
            ...
            <div className="absolute right-0 z-30 mt-2 max-h-72 w-full min-w-72 overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
              <label className="flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 font-bold">
                <input
                  type="checkbox"
                  checked={allHistoryScopesSelected}
                  onChange={toggleAllHistoryScopes}
                />
                Všetko
              </label>
              <div className="mt-2 grid gap-2">
                {historyScopeOptions.map((scope) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={selectedHistoryScopes.includes(scope.id)}
                      onChange={() => toggleHistoryScope(scope.id)}
                    />
                    {scope.label}
                  </label>
                ))}
          ...
          >
            ...
            <div className="absolute right-0 z-30 mt-2 max-h-72 w-full min-w-72 overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
              <label className="flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 font-bold">
                <input
                  type="checkbox"
                  checked={allReligionScopesSelected}
                  onChange={toggleAllReligionScopes}
                />
                Všetko
              </label>
              <div className="mt-2 grid gap-2">
                {religionScopeOptions.map((scope) => (
                  <label
                    key={scope.id}
                    className="flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 font-bold"
                  >
                    <input
                      type="checkbox"
                      checked={selectedReligionScopes.includes(scope.id)}
                      onChange={() => toggleReligionScope(scope.id)}
                    />
                    {scope.label}
                  </label>
                ))}
      ...
      {mode === 'learn' && religionActive && (
        <section className="grid gap-3">
          {filteredReligionFacts.map((fact, index) => (
            <article
              key={fact.id}
              className="rounded-xl bg-white p-4 text-xl leading-relaxed shadow"
            >
              <p className="font-bold">
                <span className="mr-2 text-slate-400">{index + 1}.</span>
                {fact.prompt}
              </p>
              <p className="mt-2 whitespace-pre-line">{fact.answer}</p>
            </article>
          ))}
        </section>
      )}
